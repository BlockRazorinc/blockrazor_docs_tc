...
在Searcher處理bundle的第4步，Searcher需要將由目標交易和套利交易組成的[bundle](../../transaction-submission/rpc/bsc/eth_sendmevbundle/)發給鏈上的所有主流Builder，確保參與競拍的區塊中都包含bundle，以提升bundle的上鏈速度。
...
bundle上鏈速度的提升涉及Builder API限流解除成本。Searcher可以免費對接RPC的[Bundle](../../transaction-submission/rpc/bsc/eth_sendmevbundle/)，RPC會將bundle轉發給鏈上的主流Builder，在節省成本的同時保障上鏈速度。
...
在bundle上鍊速度方面，Searcher可直接對接RPC的[Bundle](../../transaction-submission/rpc/bsc/eth_sendmevbundle/)，RPC會在第一時間將bundle轉發給鏈上的主流Builder，同時基於 [BEF](../../he-xin-ji-shu/blockchain-edge-fabric.md)，RPC可以在網絡層面實現交易端到端的低延遲轉發。數據表明，提交至RPC的交易在下個區塊內上鏈的概率高達95%，100%的交易可以在兩個區塊內上鏈，交易上鏈的速度和穩定性遠超其他RPC。
...
BlockRazor為Searcher提供極具性價比的訂閱計劃，一經訂閱，Searcher以極低延遲訂閱Public Mempool交易，向BSC上出塊率第一的Block Builder發送0 gwei交易，同時訂閱[Private Mempool](/broken/pages/NXkreaYlqBUKcNyRE5Zf)的隱私交易流執行backrun策略以拓展套利機會範圍。
